...
class Testing(commands.Cog):
    ...
    @commands.group(name='Testing',
                    help='Commands not fully implemented yet.')
    ...
    async def on_message(self, message):
        if message.content.lower().startswith("!executioner"):
            ...
            staff_channel = discord.utils.find(
                    lambda c: c.id == 714277622716170280,
                    self.client.get_all_channels())
            if any("Executioner" in x for x in role_names):
                if self.logged == True:
                    await staff_channel.send("<@{0} appears to be working as functional again.".format(self.scuttle))
                self.logged = False
                return

            if not self.logged:
                await staff_channel.send("Heads up, <@{0}> took over 10 seconds to add Executioner to {1}. Adding now".format(self.scuttle, message.author.mention))
                self.logged = True
            
            if self.role == None:
                new_strike = discord.utils.find(lambda m: m.name == "Executioner", message.guild.roles)
